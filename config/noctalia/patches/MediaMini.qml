...
Item {
  ...
  // Synced lyrics from mpris-lyrics plugin (falls back to artist - title)
  property int _lyricsBindTick: 0
  readonly property var lyricsService: {
    var _refresh = _lyricsBindTick;
    if (!PluginService.isPluginLoaded("mpris-lyrics"))
      return null;
    var api = PluginService.getPluginAPI("mpris-lyrics");
    return api ? api.mainInstance : null;
  }
  readonly property string displayTitle: {
    var _refresh = _lyricsBindTick;
    if (lyricsService) {
      var _rev = lyricsService.lyricRevision;
      var _state = lyricsService.fetchState;
      var _text = lyricsService.barText;
      if (lyricsService.hasActiveTrack && _text)
        return _text;
    }
    return title;
  }
  ...
  Connections {
    ...
    function onPluginLoaded(pluginId) {
      if (pluginId === "mpris-lyrics")
        root._lyricsBindTick++;
    }
